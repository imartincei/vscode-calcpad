...
export class CalcpadUIProvider implements vscode.WebviewViewProvider {
            ...
            message => {
                switch (message.type) {
                    ...
                    case 'getSettings':
                        ...
                        break;
                    case 'storeJWT':
                        // JWT storage is now handled by S3StorageProvider
                        this._outputChannel.appendLine('JWT storage request ignored - handled by S3StorageProvider');
                        break;
                    case 'storeS3JWT':
                        // Store S3 JWT for CalcPad operations
                        this._context.secrets.store('calcpad.s3.jwt', message.jwt);
                        this._outputChannel.appendLine('S3 JWT stored for CalcPad operations');
                        break;
                    case 'clearS3JWT':
                        // Clear S3 JWT
                        this._context.secrets.delete('calcpad.s3.jwt');
                        this._outputChannel.appendLine('S3 JWT cleared');
                        break;
                    case 'getS3Config':
    ...
    private _getHtmlForWebview(webview: vscode.Webview) {
        ...
        const htmlUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'src', 'webview', 'index.html'));
...
<body>
    <div class="calcpad-ui">
        ...
        <div class="tab-content" id="files-tab">
            <div id="s3-app">
                <!-- Login Form -->
                <div id="s3-login-container" class="s3-login-container">
                    <h3>S3 Storage Login</h3>
                    <div id="s3-error" class="error" style="display: none;"></div>
                    <form id="s3-login-form">
                        <div class="form-group">
                            <label>Username</label>
                            <input id="s3-username" type="text" required>
                        </div>
                        <div class="form-group">
                            <label>Password</label>
                            <input id="s3-password" type="password" required>
                        </div>
                        <button id="s3-login-btn" type="submit">Login</button>
                    </form>
                </div>

                <!-- Main S3 Application -->
                <div id="s3-main" class="s3-main" style="display: none;">
                    <div class="s3-header">
                        <span id="s3-welcome">Welcome</span>
                        <button id="s3-logout-btn">Logout</button>
                    </div>

                    <!-- S3 Files Content -->
                    <div class="s3-content">
                        <div class="s3-filters">
                            <input id="s3-search" placeholder="Search files..." style="flex: 1;">
                            <button id="s3-refresh-btn">Refresh</button>
                        </div>

                        <div id="s3-files-grid" class="s3-files-grid">
    ...
    <script>
        ...
        function setupSettingsEvents() {
            ...
            // S3 Config button
            document.getElementById('open-s3-config').addEventListener('click', function() {
                vscode.postMessage({
                    type: 'openS3Config'
                });
            });
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.type) {
                ...
                case 's3ConfigResponse':
                    // Set S3 API URL in the S3 state
                    s3State.apiUrl = message.apiUrl;
        ...
        // S3 Functions
        async function s3Login(username, password) {
            vscode.postMessage({ type: 'debug', message: '[S3Login] Checking API URL: ' + s3State.apiUrl });

            if (!s3State.apiUrl) {
                throw new Error('S3 API URL not configured');
            }

            vscode.postMessage({ type: 'debug', message: '[S3Login] Making fetch request to: ' + s3State.apiUrl + '/api/auth/login' });

            const response = await fetch(s3State.apiUrl + '/api/auth/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username, password })
            });

            vscode.postMessage({ type: 'debug', message: '[S3Login] Response status: ' + response.status });
            ...
            vscode.postMessage({ type: 'debug', message: '[S3Login] Login response received, setting state' });

            s3State.authToken = data.token;
            s3State.currentUser = data.user;
            s3State.isAuthenticated = true;

            vscode.postMessage({ type: 'debug', message: '[S3Login] State updated - authenticated: ' + s3State.isAuthenticated });
            vscode.postMessage({ type: 'debug', message: '[S3Login] User: ' + (data.user ? data.user.username : 'unknown') });

            // Store JWT for CalcPad operations
            vscode.postMessage({
                type: 'storeS3JWT',
                jwt: data.token
            });
        ...
        function s3Logout() {
            s3State.isAuthenticated = false;
            s3State.authToken = null;
            s3State.currentUser = null;
            s3State.files = [];

            // Clear JWT from CalcPad storage
            vscode.postMessage({
                type: 'clearS3JWT'
            });
        ...
        async function s3RefreshFiles() {
            if (!s3State.authToken) {
                vscode.postMessage({ type: 'debug', message: '[S3Files] No auth token, skipping file refresh' });
                return;
            }

            vscode.postMessage({ type: 'debug', message: '[S3Files] Refreshing files from: ' + s3State.apiUrl });

            try {
                const response = await fetch(s3State.apiUrl + '/api/files', {
                    headers: { 'Authorization': 'Bearer ' + s3State.authToken }
                });
                const data = await response.json();
                s3State.files = data;
                ...
                vscode.postMessage({ type: 'debug', message: '[S3Files] Error loading files: ' + error });
            }
        }

        function updateS3UI() {
            const loginContainer = document.getElementById('s3-login-container');
            const mainContainer = document.getElementById('s3-main');
            const welcomeSpan = document.getElementById('s3-welcome');

            vscode.postMessage({ type: 'debug', message: '[S3UI] updateS3UI called, authenticated: ' + s3State.isAuthenticated });
            vscode.postMessage({ type: 'debug', message: '[S3UI] loginContainer found: ' + !!loginContainer });
            vscode.postMessage({ type: 'debug', message: '[S3UI] mainContainer found: ' + !!mainContainer });

            if (s3State.isAuthenticated) {
                if (loginContainer) loginContainer.style.display = 'none';
                if (mainContainer) mainContainer.style.display = 'block';
                if (welcomeSpan) welcomeSpan.textContent = 'Welcome, ' + (s3State.currentUser?.username || 'User');
                vscode.postMessage({ type: 'debug', message: '[S3UI] Switched to main view, calling s3RefreshFiles' });
                s3RefreshFiles();
            } else {
                if (loginContainer) loginContainer.style.display = 'block';
                if (mainContainer) mainContainer.style.display = 'none';
                vscode.postMessage({ type: 'debug', message: '[S3UI] Switched to login view' });
            }
        }

        function updateFilesList() {
            const grid = document.getElementById('s3-files-grid');
            const searchQuery = document.getElementById('s3-search').value.toLowerCase();

            const filteredFiles = s3State.files.filter(file =>
            ...
            filteredFiles.forEach(file => {
                ...
                fileCard.className = 's3-file-card';
                fileCard.innerHTML = '<div class="s3-file-name">' + file.fileName + '</div>' +
        ...
        function setupS3Events() {
            vscode.postMessage({ type: 'debug', message: '[S3Setup] Setting up S3 events' });

            // Check if elements exist
            const loginForm = document.getElementById('s3-login-form');
            const logoutBtn = document.getElementById('s3-logout-btn');
            const refreshBtn = document.getElementById('s3-refresh-btn');
            const searchInput = document.getElementById('s3-search');

            vscode.postMessage({ type: 'debug', message: '[S3Setup] Elements found - form: ' + !!loginForm + ', logout: ' + !!logoutBtn + ', refresh: ' + !!refreshBtn + ', search: ' + !!searchInput });
            ...
            if (loginForm) {
                ...
                const loginBtn = document.getElementById('s3-login-btn');
                const errorDiv = document.getElementById('s3-error');
                ...
                try {
                    ...
                    await s3Login(username, password);
                    vscode.postMessage({ type: 'debug', message: '[S3Login] Login successful, calling updateS3UI' });
                    updateS3UI();
                } catch (error) {
                    vscode.postMessage({ type: 'debug', message: '[S3Login] Login failed: ' + error.message });
            ...
            // Logout button
            document.getElementById('s3-logout-btn').addEventListener('click', s3Logout);

            // Refresh button
            document.getElementById('s3-refresh-btn').addEventListener('click', s3RefreshFiles);

            // Search input
            document.getElementById('s3-search').addEventListener('input', updateFilesList);

            // Request S3 config
            vscode.postMessage({ type: 'getS3Config' });
        }

        // Initialize S3 on page load (with delay to ensure DOM is ready)
        vscode.postMessage({ type: 'debug', message: '[S3Init] Initializing S3 app' });
        setTimeout(() => {
            vscode.postMessage({ type: 'debug', message: '[S3Init] Setting up S3 events after delay' });
            setupS3Events();
        }, 100);
